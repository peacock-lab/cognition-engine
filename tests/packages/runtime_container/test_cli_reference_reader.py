...
from runtime_container.cli_reference_reader import (
    ...
    cli_reference_read_status_dict,
    read_cli_reference,
)
...
def test_reference_reader_reads_local_markdown_with_low_risk(tmp_path) -> None:
    ...
    status = cli_reference_read_status_dict(result)
...
def test_reference_reader_accepts_repo_relative_path_under_named_allowed_root(
    ...
    result = read_cli_reference(
        CliReferenceReadRequestCandidate(reference="docs/guide.md", policy=policy)
    )
...
def test_reference_reader_blocks_absolute_path_outside_allowed_root(tmp_path) -> None:
    ...
    result = read_cli_reference(
        CliReferenceReadRequestCandidate(reference=str(outside), policy=policy)
    )
...
def test_reference_reader_blocks_parent_traversal(tmp_path) -> None:
    ...
    result = read_cli_reference(
        CliReferenceReadRequestCandidate(reference="../outside.md", policy=policy)
    )
...
def test_reference_reader_blocks_forbidden_suffix_and_segment(tmp_path) -> None:
    ...
    suffix_result = read_cli_reference(
        CliReferenceReadRequestCandidate(reference="script.py", policy=policy)
    )
    segment_result = read_cli_reference(
        CliReferenceReadRequestCandidate(reference="node_modules/readme.md", policy=policy)
    )
...
def test_reference_reader_blocks_forbidden_path_marker(tmp_path) -> None:
    ...
    result = read_cli_reference(
        CliReferenceReadRequestCandidate(reference=".env.example.md", policy=policy)
    )
...
def test_reference_reader_redacts_sensitive_lines_from_excerpt(tmp_path) -> None:
    ...
    result = read_cli_reference(
        CliReferenceReadRequestCandidate(reference="notes.md", policy=policy)
    )
    status = cli_reference_read_status_dict(result)
...
def test_reference_reader_applies_bounded_output_budgets(tmp_path) -> None:
    ...
    result = read_cli_reference(
        CliReferenceReadRequestCandidate(reference="long.md", policy=policy)
    )
