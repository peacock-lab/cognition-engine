...
from cognition_task_workflows._agents.workflow_admission import (
...
from cognition_task_workflows._agents.workflow_registry_observation import (
    build_twf_agent_workflow_candidate_descriptor_source,
    build_twf_agent_workflow_registry_observation,
    twf_agent_workflow_candidate_descriptor_source_status_dict,
    twf_agent_workflow_registry_observation_status_dict,
)
from cognition_task_workflows._requests.registry import (
...
def test_passed_agent_gate_builds_candidate_visible_observation() -> None:
    ...
    source = build_twf_agent_workflow_candidate_descriptor_source(gate)
    observation = build_twf_agent_workflow_registry_observation(
        gate,
        source=source,
    )
    source_status = twf_agent_workflow_candidate_descriptor_source_status_dict(
        source
    )
    observation_status = twf_agent_workflow_registry_observation_status_dict(
        observation
    )
...
def test_blocked_agent_gate_builds_blocked_observation() -> None:
    ...
    observation = build_twf_agent_workflow_registry_observation(gate)
...
def test_observation_does_not_modify_default_registry() -> None:
    ...
    before = tuple(
        ...
        for descriptor in list_twf_descriptors(registry)
    )
...
def test_candidate_descriptor_source_refs_are_stable_and_candidate_only() -> None:
    ...
    assert source.source_name == (
        "twf-research-agent-workflow-candidate-descriptor-source"
    )
    assert source.agent_workflow_descriptor_ref == (
        "agent-workflow-descriptor://twf-research-agent-workflow"
    )
    ...
    assert source.evidence_projection_ref == (
        ...
        "twf-research-agent-workflow/research-team"
    )
...
def _passed_agent_gate():
    descriptor = build_agent_workflow_descriptor(
        workflow_name="twf_research_agent_workflow",
