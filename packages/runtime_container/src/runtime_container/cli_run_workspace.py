"""Governed run workspace helpers for CLI task workflows."""
...
CLI_RUN_WORKSPACE_SCHEMA_VERSION = "cli-run-workspace.v1"
...
CLI_RUN_WORKSPACE_RETENTION_POLICIES = frozenset(
    {"keep", "ephemeral", "delete_on_success"}
)
CLI_RUN_WORKSPACE_CLEANUP_POLICIES = frozenset(
    {"manual", "delete_on_success", "delete_always"}
)
CLI_RUN_WORKSPACE_FORBIDDEN_PATH_MARKERS = (
    ".env",
    ".netrc",
    ".npmrc",
    ".pypirc",
...
class CliRunWorkspacePolicyCandidate:
    ...
    allowed_subdirs: tuple[str, ...] = CLI_RUN_WORKSPACE_SUBDIRS
    metadata: dict[str, Any] = field(default_factory=dict)


@dataclass(frozen=True)
class CliRunWorkspaceStateCandidate:
...
@dataclass(frozen=True)
class CliRunWorkspaceWriteCandidate:
...
@dataclass(frozen=True)
class CliRunWorkspaceCleanupCandidate:
...
def build_cli_run_workspace_policy(
    *,
    workspace_root: str | Path,
    retention_policy: str = "keep",
    cleanup_policy: str = "manual",
    max_write_bytes: int = 65536,
) -> CliRunWorkspacePolicyCandidate:
    """Build a run workspace policy without creating files."""

    return CliRunWorkspacePolicyCandidate(
        ...
        metadata={
            ...
            "workspace_schema_version": CLI_RUN_WORKSPACE_SCHEMA_VERSION,
        },
    )


def create_cli_run_workspace(
    *,
    policy: CliRunWorkspacePolicyCandidate,
    workflow_name: str,
    run_id: str,
    ...
    if blocking:
        return CliRunWorkspaceStateCandidate(
            ...
            metadata={"workspace_schema_version": CLI_RUN_WORKSPACE_SCHEMA_VERSION},
        )
    ...
    state = CliRunWorkspaceStateCandidate(
        workspace_ref=f"run-workspace://{workflow_slug}/{run_slug}",
        workspace_path=str(workspace_path),
        workflow_name=workflow_slug,
        run_id=run_slug,
...
def write_cli_run_workspace_json(
    workspace: CliRunWorkspaceStateCandidate,
    *,
    relative_path: str,
    payload: Mapping[str, Any],
    kind: str,
    max_write_bytes: int = 65536,
) -> tuple[CliRunWorkspaceStateCandidate, CliRunWorkspaceWriteCandidate]:
    """Write bounded JSON into a workspace and attach the produced ref."""

    text = json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True) + "\n"
    return write_cli_run_workspace_text(
        workspace,
...
def write_cli_run_workspace_text(
    workspace: CliRunWorkspaceStateCandidate,
    *,
    relative_path: str,
    text: str,
    ...
    if blocking:
        write_result = CliRunWorkspaceWriteCandidate(
...
def finalize_cli_run_workspace(
    workspace: CliRunWorkspaceStateCandidate,
    *,
    status: str,
...
def cleanup_cli_run_workspace(
    workspace: CliRunWorkspaceStateCandidate,
    *,
    status: str,
) -> tuple[CliRunWorkspaceStateCandidate, CliRunWorkspaceCleanupCandidate]:
    ...
    if not should_delete:
        cleanup = CliRunWorkspaceCleanupCandidate(
...
def cli_run_workspace_status_dict(
    workspace: CliRunWorkspaceStateCandidate | None,
) -> dict[str, Any] | None:
...
def _validate_policy(policy: CliRunWorkspacePolicyCandidate) -> list[str]:
    blocking: list[str] = []
    if policy.retention_policy not in CLI_RUN_WORKSPACE_RETENTION_POLICIES:
        blocking.append("workspace_retention_policy_invalid")
    if policy.cleanup_policy not in CLI_RUN_WORKSPACE_CLEANUP_POLICIES:
...
def _resolve_workspace_relative_path(
    ...
    if any(marker in path_text for marker in CLI_RUN_WORKSPACE_FORBIDDEN_PATH_MARKERS):
        return None
...
def _write_manifest(
    ...
    *,
    ...
    payload = {
        "schema_version": CLI_RUN_WORKSPACE_SCHEMA_VERSION,
        "workspace_ref": workspace.workspace_ref,
        "workspace_path": workspace.workspace_path,
