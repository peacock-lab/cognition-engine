...
from runtime_container.cli_plan_workflow import (
    ...
    extract_cli_plan_requirements,
    format_cli_plan_for_terminal,
    review_cli_plan_quality,
...
def test_requirement_extraction_preserves_fishpond_entities_and_constraints() -> None:
    facts = extract_cli_plan_requirements(
        CliPlanWorkflowRequestCandidate(
...
def test_terminal_display_preserves_line_breaks() -> None:
    facts = extract_cli_plan_requirements(
        CliPlanWorkflowRequestCandidate(user_text="鱼塘500平米，设计建设方案")
    )
...
def test_quality_review_identifies_entity_and_scale_missing() -> None:
    ...
    review = review_cli_plan_quality(formatted_without_facts, facts)
...
def test_controlled_live_workflow_uses_one_model_call_and_preserves_facts() -> None:
    ...
    assert service.requests[0].metadata["interaction_mode"] == "cli_plan_workflow"
