"""Shared CLI task-control candidates for governed terminal workflows."""
...
class CliTaskPreflightCandidate:
    """Preflight facts for a governed CLI task workflow."""
...
@dataclass(frozen=True)
class CliTaskWorkspaceCandidate:
...
@dataclass(frozen=True)
class CliTaskRunContextCandidate:
    """Task-control context shared by CLI workflows."""

    workflow_name: str
    task_kind: str
    run_id: str
    stages: tuple[str, ...]
    preflight: CliTaskPreflightCandidate
    workspace: CliTaskWorkspaceCandidate
...
def build_cli_task_run_id(
    workflow_name: str,
    *,
    session_id: str | None = None,
    turn_index: int | None = None,
) -> str:
    """Build a stable, display-safe run id for CLI task contexts."""
...
def evaluate_cli_task_preflight(
    *,
...
) -> CliTaskPreflightCandidate:
    """Evaluate generic CLI task preflight without calling external services."""
    ...
    return CliTaskPreflightCandidate(
...
def build_cli_task_workspace_candidate(
    *,
    workflow_name: str,
    run_id: str,
...
def build_cli_task_run_context(
    *,
    workflow_name: str,
    task_kind: str,
    session_id: str | None = None,
    ...
    """Build the shared task-control context for a CLI workflow run."""

    resolved_live_gate = live_gate or (
        "controlled_live" if live_model_allowed else "no_live"
    )
    run_id = build_cli_task_run_id(
        workflow_name,
    ...
    workspace = build_cli_task_workspace_candidate(
    ...
    return CliTaskRunContextCandidate(
        workflow_name=workflow_name,
        task_kind=task_kind,
        run_id=run_id,
        stages=CLI_TASK_CONTROL_STAGES,
        preflight=preflight,
        workspace=workspace,
...
def finalize_cli_task_run_context(
    context: CliTaskRunContextCandidate,
    *,
    status: str,
    artifact_refs: Sequence[str] = (),
    evidence_refs: Sequence[str] = (),
...
def cli_task_run_context_status_dict(
    context: CliTaskRunContextCandidate,
) -> dict[str, Any]:
