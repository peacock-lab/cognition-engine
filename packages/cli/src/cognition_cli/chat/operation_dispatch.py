"""Chat operation flow dispatch for the Cognition System CLI."""
...
from cognition_cli.chat.routing import (
    _chat_product_gateway_operation_flow_route_projection,
    _chat_operation_flow_route_from_product_gateway_projection,
)
...
from cognition_cli.chat.operation_flows import (
    _dispatch_chat_operation_flow_turn,
)
...
def _dispatch_chat_input_turn(
    *,
    ...
    external_bridge_result = build_chat_external_readonly_bridge_turn(
        ...
        initial_ask_runner=external_readonly_ask_initial_runner,
        follow_up_ask_runner=external_readonly_ask_follow_up_runner,
    )
    ...
    operation_route_projection = _chat_product_gateway_operation_flow_route_projection(
        args=args,
        user_text=user_text,
        chat_session_id=chat_session_id,
        turn_index=turn_index,
    ...
    operation_route = _chat_operation_flow_route_from_product_gateway_projection(
        operation_route_projection
    )
    operation_flow_result = _dispatch_chat_operation_flow_turn(
        args=args,
        user_text=user_text,
        chat_session_id=chat_session_id,
        turn_index=turn_index,
    ...
    if operation_flow_result.handled:
        return _chat_turn_from_operation_flow_result(operation_flow_result)
...
def _chat_session_operation_summary_text(
    ...
    question = _first_matching_user_text(
        ...
        or _looks_like_external_readonly_question_text(text),
    )
    if question:
    ...
    elif _has_successful_external_readonly_answer(history):
        operations.append("完成首轮 external-readonly 问答并形成资料答案。")
    ...
    for follow_up in _recent_follow_up_history_items(history):
...
def _looks_like_external_readonly_question_text(value: str) -> bool:
    normalized = "".join(value.strip().split()).lower()
    if not normalized:
        return False
    if _URL_RE.fullmatch(value.strip()):
        return False
    if normalized in {"1", "2", "y", "yes", "n", "no", "同意"}:
        return False
    if _looks_like_session_operation_summary(value):
        return False
    return any(
        keyword in normalized
        for keyword in (
...
def _recent_follow_up_history_items(
    history: MutableSequence[dict[str, str]],
) -> list[dict[str, str]]:
    ...
    for item in history:
        ...
        if item.get("follow_up") == "true":
            ...
            for keyword in ("适用", "适合", "场景", "用途", "哪些用户")
        ):
            items.append(item)
    return items[-3:]
...
def _chat_turn_from_operation_flow_result(
    operation_flow_result: Any,
) -> ChatTurnDispatchResult:
    return ChatTurnDispatchResult(
        exit_code=operation_flow_result.exit_code,
        latest_plan_display_text=operation_flow_result.latest_plan_display_text,
        latest_plan_snapshot=operation_flow_result.latest_plan_snapshot,
    )
