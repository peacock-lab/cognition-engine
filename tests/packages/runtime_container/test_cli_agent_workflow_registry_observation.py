...
from runtime_container.cli_agent_workflow_registry_observation import (
    build_cli_agent_workflow_candidate_descriptor_source,
    build_cli_agent_workflow_registry_observation,
    cli_agent_workflow_candidate_descriptor_source_status_dict,
    cli_agent_workflow_registry_observation_status_dict,
)
from runtime_container.cli_task_workflow_registry import (
    ...
    list_cli_task_workflow_descriptors,
)
...
def test_passed_agent_gate_builds_candidate_visible_observation() -> None:
    ...
    source = build_cli_agent_workflow_candidate_descriptor_source(gate)
    observation = build_cli_agent_workflow_registry_observation(
        gate,
        source=source,
    )
    source_status = cli_agent_workflow_candidate_descriptor_source_status_dict(
        source
    )
    observation_status = cli_agent_workflow_registry_observation_status_dict(
        observation
    )
...
def test_blocked_agent_gate_builds_blocked_observation() -> None:
    ...
    observation = build_cli_agent_workflow_registry_observation(gate)
...
def test_observation_does_not_modify_default_registry() -> None:
    ...
    before = tuple(
        ...
        for descriptor in list_cli_task_workflow_descriptors(registry)
    )
...
def test_candidate_descriptor_source_refs_are_stable_and_candidate_only() -> None:
    ...
    assert source.source_name == (
        "cli-research-agent-workflow-candidate-descriptor-source"
    )
    assert source.agent_workflow_descriptor_ref == (
        "agent-workflow-descriptor://cli-research-agent-workflow"
    )
    ...
    assert source.evidence_projection_ref == (
        ...
        "cli-research-agent-workflow/research-team"
    )
