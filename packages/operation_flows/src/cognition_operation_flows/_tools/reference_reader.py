...
def build_default_reference_reader_policy(
    *,
    ...
    allowed_files: Sequence[str | Path] | None = None,
    ...
    normalized_files = tuple(
        str(Path(file).expanduser().resolve())
...
def read_twf_reference(
    ...
    return TwfReferenceReadResultCandidate(
        ...
        metadata={
            ...
            "allowed_files": list(request.policy.allowed_files),
...
def _resolve_reference_path(
    ...
    if not candidate_paths:
        candidate_paths = tuple(
            _relative_reference_candidates(candidate, allowed_roots, allowed_files)
        )
    first_allowed_path: Path | None = None
    for candidate_path in candidate_paths:
        resolved = candidate_path.resolve()
        if _is_allowed_reference_path(resolved, allowed_roots, allowed_files):
            if resolved.exists():
                return resolved
...
def _relative_reference_candidates(
    ...
    for allowed_file in allowed_files:
        allowed_path = Path(allowed_file).expanduser().resolve()
        if allowed_path.name == candidate_text or allowed_path.as_posix().endswith(
            f"/{candidate_text}"
        ):
...
def _is_allowed_file(path: Path, allowed_files: Sequence[str]) -> bool:
