...
from cognition_operation_flows._tools.exposure_profile import resolve_operation_flow_tool_exposure_profile
...
from cognition_operation_flows._tools.toolset_admission import OperationFlowToolOperationFactsCandidate


def test_default_reference_reader_passes_loading_validation(tmp_path) -> None:
    resolution = resolve_operation_flow_tool_exposure_profile(repo_root=tmp_path)

    gate = validate_operation_flow_tool_loading_gate(resolution)
    status = operation_flow_tool_loading_gate_status_dict(gate)
...
def test_external_readonly_tool_passes_with_confirmation_and_boundaries(
    ...
    gate = validate_operation_flow_tool_loading_gate(
        resolution,
...
def _resolve_external_readonly_tool(tmp_path):
    return resolve_operation_flow_tool_exposure_profile(
        profile_name="external_readonly",
        repo_root=tmp_path,
