...
from product_runtime_assembly import operation_flow_provider_factory
...
def test_operation_flow_default_provider_factory_is_lazy_runtime_container_wrapper(
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(
        operation_flow_provider_factory,
        "_build_runtime_container_llm_invocation_service_factory",
        fake_builder,
    )

    factory = build_operation_flow_default_llm_invocation_service_factory(
        metadata={"request_id": "operation_flow-provider-factory-test"},
    )
    ...
    assert isinstance(factory, ProductRuntimeAssemblyOperationFlowLlmInvocationServiceFactory)
    assert captured_metadata == {
        ...
        "request_id": "operation_flow-provider-factory-test",
    }
    assert runtime_factory.captured["config_context"] == {"config": "context"}
    assert runtime_factory.captured["config_selection"].config_root == "config/operation_flow"
...
def test_operation_flow_default_provider_factory_exports_are_explicit() -> None:
    assert operation_flow_provider_factory.__all__ == [
        "DEFAULT_OPERATION_FLOW_LLM_INVOCATION_PROVIDER_FACTORY_REF",
        "ProductRuntimeAssemblyOperationFlowLlmInvocationServiceFactory",
        "build_operation_flow_default_llm_invocation_service_factory",
    ]
