...
def test_audit_existing_workspace_path_without_model_or_tool_call(
    ...
    result = run_operation_flow_run_workspace_evidence_audit_workflow(
        OperationFlowRunWorkspaceEvidenceAuditWorkflowRequestCandidate(
...
def test_audit_output_workspace_writes_summary_layers(tmp_path: Path) -> None:
    ...
    assert result_payload["workflow"] == OPERATION_FLOW_RUN_WORKSPACE_EVIDENCE_AUDIT_WORKFLOW_NAME
...
def _create_passed_workspace(workspace_root: Path) -> Path:
    ...
    workspace, _ = write_operation_flow_run_workspace_json(
        workspace,
    ...
    workspace, _ = write_operation_flow_run_workspace_text(
        workspace,
    ...
    workspace = finalize_operation_flow_run_workspace(
        workspace,
        status="succeeded",
