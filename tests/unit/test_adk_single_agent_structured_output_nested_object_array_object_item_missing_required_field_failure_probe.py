#!/usr/bin/env python3
"""ADK 单 agent 结构化输出嵌套对象中的数组对象项缺少必填字段失败路径最小验证脚本测试。"""
...
from engine.analyzer.probe_adk_single_agent_structured_output_nested_object_array_object_item_missing_required_field_failure import (  # noqa: E402
    AGENT_NAME,
    APP_NAME,
...
def test_extract_node_name_strips_run_id() -> None:
    assert extract_node_name(
        "single_agent_structured_output_nested_object_array_object_item_missing_required_field_failure_node@1"
    ) == "single_agent_structured_output_nested_object_array_object_item_missing_required_field_failure_node"
    assert extract_node_name(
        "wf@1/single_agent_structured_output_nested_object_array_object_item_missing_required_field_failure_node@2"
    ) == "single_agent_structured_output_nested_object_array_object_item_missing_required_field_failure_node"
...
def test_serialize_event_extracts_text_without_state_delta() -> None:
    event = SimpleNamespace(
        ...
        node_info=SimpleNamespace(
            path="single_agent_structured_output_nested_object_array_object_item_missing_required_field_failure_node@1"
        ),
    )
...
def test_build_success_payload_marks_nested_object_array_object_item_missing_required_field_failure_path_acceptance() -> None:
    payload = build_success_payload(
        ...
        error_message=(
            ...
            "metadata.characters.1.role\n"
            "  Field required [type=missing, input_value={'name': 'Nia'}, input_type=dict]"
        ),
        event_records=[
            {
                "author": AGENT_NAME,
                "node_path": "single_agent_structured_output_nested_object_array_object_item_missing_required_field_failure_node@1",
                "node_name": "single_agent_structured_output_nested_object_array_object_item_missing_required_field_failure_node",
                "output": None,
                "role": "model",
    ...
    assert (
        payload[
            "accepted_as_minimal_single_agent_structured_output_nested_object_array_object_item_missing_required_field_failure_path"
        ]
        is True
    )
    ...
    assert payload[
        "entered_schema_nested_object_array_object_item_missing_required_field_failure_semantics"
    ] is True
    assert payload["nested_object_array_object_item_missing_required_field_failure_observed"] is True
...
def test_build_failure_payload_keeps_boundary_warning() -> None:
    payload = build_failure_payload(
        ...
        input_text="Write a tiny story outline with metadata characters as structured output.",
        exc=RuntimeError(
            "schema nested object array object item missing required field validation was not observed"
        ),
    )
    ...
    assert payload["selected_runtime_path"] == SELECTED_RUNTIME_PATH
    assert "schema 嵌套对象中的数组对象项缺少必填字段失败承接层" in payload["boundary_judgement"]
