...
from runtime_container.cli_tool_exposure_profile import resolve_cli_tool_exposure_profile
...
def test_default_reference_reader_passes_loading_validation(tmp_path) -> None:
    ...
    gate = validate_cli_tool_loading_gate(resolution)
    status = cli_tool_loading_gate_status_dict(gate)
...
def test_external_readonly_tool_passes_with_confirmation_and_boundaries(
    ...
    gate = validate_cli_tool_loading_gate(
        resolution,
...
def _resolve_external_readonly_tool(tmp_path):
    return resolve_cli_tool_exposure_profile(
        profile_name="external_readonly",
        repo_root=tmp_path,
