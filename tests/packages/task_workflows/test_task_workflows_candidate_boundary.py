...
TASK_WORKFLOWS_PACKAGE_ROOT = SOURCE_ROOT / "cognition_task_workflows"

sys.path.insert(0, str(SOURCE_ROOT))

import cognition_task_workflows  # noqa: E402
from cognition_task_workflows._core.descriptors import (  # noqa: E402
    TaskWorkflowDescriptorCandidate,
    build_task_workflow_descriptor_candidate,
)
from cognition_task_workflows._core.registry import (  # noqa: E402
    build_task_workflow_registry_candidate,
    resolve_task_workflow_descriptor_candidate,
)


def test_task_workflow_descriptor_defaults_to_candidate_channel_neutral() -> None:
    descriptor = build_task_workflow_descriptor_candidate(
    ...
    assert descriptor.owner_package == "cognition_task_workflows"
...
def test_task_workflow_descriptor_rejects_boundary_drift(
    field_name: str,
    value: bool,
) -> None:
    payload = {
    ...
    with pytest.raises(ValidationError):
        TaskWorkflowDescriptorCandidate.model_validate(payload)


def test_task_workflow_descriptor_rejects_raw_payload_metadata() -> None:
    with pytest.raises(ValidationError):
        build_task_workflow_descriptor_candidate(
...
def test_task_workflow_registry_resolves_descriptors_without_execution() -> None:
    descriptor = build_task_workflow_descriptor_candidate(
    ...
    registry = build_task_workflow_registry_candidate([descriptor])

    resolved = resolve_task_workflow_descriptor_candidate(
        registry,
    ...
    assert resolve_task_workflow_descriptor_candidate(registry, "missing") is None


def test_task_workflow_registry_rejects_duplicate_workflow_ids() -> None:
    descriptor = build_task_workflow_descriptor_candidate(
    ...
    with pytest.raises(ValidationError):
        build_task_workflow_registry_candidate([descriptor, descriptor])


def test_task_workflows_root_public_surface_is_minimal() -> None:
    ...
    assert cognition_task_workflows.__all__ == ()
    for symbol_name in forbidden_root_symbols:
        assert not hasattr(cognition_task_workflows, symbol_name), symbol_name


def test_task_workflows_package_is_channel_neutral_and_release_configured() -> None:
    pyproject = tomllib.loads((PACKAGE_ROOT / "pyproject.toml").read_text())
    tool_config = pyproject["tool"]["cognition_task_workflows"]

    assert pyproject["project"]["name"] == "cognition-system-task-workflows"
    assert tool_config["status"] == "channel_neutral_task_workflow_layer"
    assert tool_config["channel_neutral"] is True
    assert tool_config["workflow_execution_enabled"] is True
    assert tool_config["route_descriptor_candidate_enabled"] is True
    assert tool_config["twf_router_candidate_enabled"] is True
    assert tool_config["task_control_candidate_enabled"] is True
...
def test_task_workflows_in_v070_root_package_and_release_list() -> None:
    ...
    assert "cognition-system-task-workflows==0.8.0" in root_pyproject[
        "project"
    ]["dependencies"]
    assert '"cognition-system-task-workflows"' in release_script
    assert '"packages/task_workflows"' in release_script
...
def test_external_readonly_private_subpackage_layout_and_boundaries() -> None:
    ...
    assert tuple(TASK_WORKFLOWS_PACKAGE_ROOT.glob("twf_external_readonly_*.py")) == ()
    for source_root in external_boundary_roots:
        for source_path in source_root.rglob("*.py"):
            source = source_path.read_text(encoding="utf-8")
            assert "cognition_task_workflows._external_readonly" not in source, (
                source_path,
...
def test_tools_skills_agents_private_subpackage_layout_and_boundaries() -> None:
    ...
    removed_root_modules = (
        "twf_readonly_tool_design.py",
        "twf_reference_reader.py",
        "twf_tool_exposure_profile.py",
        "twf_tool_loading_validation.py",
        "twf_toolset_admission.py",
        "skill_projection_context.py",
        "twf_skill_capability_projection.py",
        "twf_skill_registry_admission.py",
        "twf_agent_workflow_admission.py",
        "twf_agent_workflow_registry_observation.py",
    )
    ...
    for exported_name in cognition_task_workflows.__all__:
        assert "AgentWorkflow" not in exported_name
        assert not exported_name.startswith("TwfAgent")
    for source_root in external_boundary_roots:
        for source_path in source_root.rglob("*.py"):
            source = source_path.read_text(encoding="utf-8")
...
def test_workflows_requests_core_llm_private_layout_and_boundaries() -> None:
    ...
    removed_root_modules = (
        ...
        "registry.py",
        "run_workspace.py",
        "twf_config_profile_explain_workflow.py",
        "twf_llm_invocation.py",
        "twf_plan_workflow.py",
        "twf_reference_review_workflow.py",
        "twf_registry.py",
        "twf_request_builder.py",
        "twf_request_drafts.py",
        "twf_run_workspace_evidence_audit_workflow.py",
    )
    forbidden_private_import_markers = (
