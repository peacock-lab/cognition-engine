...
def test_audit_existing_workspace_path_without_model_or_tool_call(
    ...
    result = run_cli_run_workspace_evidence_audit_workflow(
        CliRunWorkspaceEvidenceAuditWorkflowRequestCandidate(
...
def test_audit_output_workspace_writes_summary_layers(tmp_path: Path) -> None:
    ...
    assert result_payload["workflow"] == CLI_RUN_WORKSPACE_EVIDENCE_AUDIT_WORKFLOW_NAME
...
def _create_passed_workspace(workspace_root: Path) -> Path:
    ...
    workspace = create_cli_run_workspace(
        policy=policy,
        workflow_name="cli_config_profile_explain_workflow",
        run_id="cli-config-profile-explain-workflow-audit-source-turn-001",
    )
    workspace, _ = write_cli_run_workspace_json(
        workspace,
        ...
        payload={
            ...
            "run_workspace_summary": {"source": "cli_explicit_args"},
        },
        kind="evidence",
    )
    workspace, _ = write_cli_run_workspace_text(
        workspace,
    ...
    workspace, _ = write_cli_run_workspace_json(
        ...
        payload={
            "workflow": "cli_config_profile_explain_workflow",
            "status": "succeeded",
            "model_call_count": 0,
            "no_live": True,
        },
    ...
    workspace = finalize_cli_run_workspace(
        workspace,
        status="succeeded",
        metadata={"workflow": "cli_config_profile_explain_workflow"},
    )
