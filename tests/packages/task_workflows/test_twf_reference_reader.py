...
def test_reference_reader_reads_local_markdown_with_low_risk(tmp_path) -> None:
    ...
    status = twf_reference_read_status_dict(result)
...
def test_reference_reader_accepts_repo_relative_path_under_named_allowed_root(
    ...
    result = read_twf_reference(
        TwfReferenceReadRequestCandidate(reference="docs/guide.md", policy=policy)
    )
...
def test_reference_reader_blocks_absolute_path_outside_allowed_root(tmp_path) -> None:
    ...
    result = read_twf_reference(
        TwfReferenceReadRequestCandidate(reference=str(outside), policy=policy)
    )
...
def test_reference_reader_reads_explicit_allowed_file_outside_allowed_roots(
    ...
    result = read_twf_reference(
        TwfReferenceReadRequestCandidate(reference=str(project_file), policy=policy)
    )
...
def test_reference_reader_blocks_parent_traversal(tmp_path) -> None:
    ...
    result = read_twf_reference(
        TwfReferenceReadRequestCandidate(reference="../outside.md", policy=policy)
    )
...
def test_reference_reader_blocks_forbidden_suffix_and_segment(tmp_path) -> None:
    ...
    suffix_result = read_twf_reference(
        TwfReferenceReadRequestCandidate(reference="script.py", policy=policy)
    )
    segment_result = read_twf_reference(
        TwfReferenceReadRequestCandidate(reference="node_modules/readme.md", policy=policy)
    )
...
def test_reference_reader_blocks_forbidden_path_marker(tmp_path) -> None:
    ...
    result = read_twf_reference(
        TwfReferenceReadRequestCandidate(reference=".env.example.md", policy=policy)
    )
...
def test_reference_reader_redacts_sensitive_lines_from_excerpt(tmp_path) -> None:
    ...
    result = read_twf_reference(
        TwfReferenceReadRequestCandidate(reference="notes.md", policy=policy)
    )
    status = twf_reference_read_status_dict(result)
...
def test_reference_reader_applies_bounded_output_budgets(tmp_path) -> None:
    ...
    result = read_twf_reference(
        TwfReferenceReadRequestCandidate(reference="long.md", policy=policy)
    )
