"""Candidate-only CLI toolset admission and tool risk review helpers."""
...
class CliToolsetAdmissionCandidate:
    """Candidate admission review for a dynamic or static CLI toolset."""
...
@dataclass(frozen=True)
class CliToolOperationFactsCandidate:
...
@dataclass(frozen=True)
class CliToolRiskReviewCandidate:
    """Risk review for one tool operation before CLI exposure."""
...
@dataclass(frozen=True)
class CliToolCandidate:
    """Candidate tool exposure after selection and risk review."""

    tool_name: str
    selected: bool
    exposed: bool
    operation_facts: CliToolOperationFactsCandidate
    risk_review: CliToolRiskReviewCandidate
    metadata: dict[str, Any] = field(default_factory=dict)


@dataclass(frozen=True)
class CliToolsetInventoryCandidate:
    """Sanitized toolset inventory after admission and per-tool review."""

    admission: CliToolsetAdmissionCandidate
    tools: tuple[CliToolCandidate, ...]
...
def evaluate_cli_toolset_admission(
    *,
    toolset_name: str,
    toolset_kind: str,
    source_ref: str | None = None,
    ...
    return CliToolsetAdmissionCandidate(
        ...
        metadata={
            "stages": list(CLI_TOOLSET_CONTROL_STAGES),
            "candidate_only": True,
...
def review_cli_tool_operation_risk(
    operation_facts: CliToolOperationFactsCandidate,
) -> CliToolRiskReviewCandidate:
    """Review one operation for readonly CLI exposure."""
    ...
    return CliToolRiskReviewCandidate(
...
def build_cli_toolset_inventory(
    admission: CliToolsetAdmissionCandidate,
    operations: Sequence[CliToolOperationFactsCandidate],
    ...
    return CliToolsetInventoryCandidate(
        admission=admission,
        tools=tuple(tools),
...
def cli_toolset_inventory_status_dict(
