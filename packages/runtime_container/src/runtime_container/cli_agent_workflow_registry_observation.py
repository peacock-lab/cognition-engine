"""Observation slot candidates for CLI agent workflow registry visibility."""
...
class CliAgentWorkflowCandidateDescriptorSource:
...
@dataclass(frozen=True)
class CliAgentWorkflowRegistryObservationCandidate:
...
def build_cli_agent_workflow_candidate_descriptor_source(
    ...
    *,
    ...
    return CliAgentWorkflowCandidateDescriptorSource(
        ...
        metadata={
            ...
            "observation_source": (
                "runtime_container.cli_agent_workflow_registry_observation"
            ),
...
def build_cli_agent_workflow_registry_observation(
    gate: AgentWorkflowLoadingGateCandidate,
    *,
    source: CliAgentWorkflowCandidateDescriptorSource | None = None,
    observation_version: str = "v0.7.0-candidate",
) -> CliAgentWorkflowRegistryObservationCandidate:
    """Build a registry observation slot without touching the registry."""

    descriptor_source = source or build_cli_agent_workflow_candidate_descriptor_source(
        gate
    )
    descriptor = gate.descriptor
    admission = gate.admission
    ...
    return CliAgentWorkflowRegistryObservationCandidate(
        observation_name=(
...
def cli_agent_workflow_candidate_descriptor_source_status_dict(
    source: CliAgentWorkflowCandidateDescriptorSource,
) -> dict[str, Any]:
...
def cli_agent_workflow_registry_observation_status_dict(
    observation: CliAgentWorkflowRegistryObservationCandidate,
) -> dict[str, Any]:
