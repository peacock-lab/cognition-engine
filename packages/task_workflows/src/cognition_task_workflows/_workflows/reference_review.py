...
from cognition_task_workflows._workflows.plan import (
...
from cognition_task_workflows._tools.exposure_profile import (
...
from cognition_task_workflows._llm.invocation import (
    TwfLlmInvocationFacade,
    build_twf_llm_invocation_request,
)
from cognition_task_workflows._skills.capability_projection import (
        ...
        if isinstance(value, list | tuple):
            ...
            for item in value:
                if isinstance(item, Mapping):
                    ...
                    continue
                item_text = _compact_text(str(item))
                if item_text:
...
def _terminology_display_line_from_mapping(item: Mapping[str, Any]) -> str:
    ...
    return f"{category_prefix}{display_term}{original}：{note_text}"
...
def _build_reference_review_task_context(
    ...
    return build_twf_run_context(
        ...
        metadata={
            "source": "cognition_task_workflows._workflows.reference_review",
