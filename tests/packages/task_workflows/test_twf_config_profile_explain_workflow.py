...
from cognition_task_workflows._workflows.config_profile_explain import (
...
def test_config_profile_explain_detector_matches_config_intent() -> None:
    assert detect_twf_config_profile_explain_request(
    ...
    assert (
        detect_twf_config_profile_explain_request("请修改配置文件，打开 live LLM")
        is False
    )
    assert (
        detect_twf_config_profile_explain_request("请打开 Agent runtime")
        is False
    )


def test_no_live_config_profile_explain_outputs_sanitized_config_context() -> None:
    result = run_twf_config_profile_explain_workflow(
        TwfConfigProfileExplainWorkflowRequestCandidate(
...
def test_config_profile_explain_run_workspace_writes_evidence(
    ...
    assert result_payload["workflow"] == TWF_CONFIG_PROFILE_EXPLAIN_WORKFLOW_NAME
