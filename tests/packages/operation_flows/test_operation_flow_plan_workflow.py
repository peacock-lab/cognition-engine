...
from cognition_operation_flows._workflows.plan import (
    ...
    extract_operation_flow_plan_requirements,
    format_operation_flow_plan_for_terminal,
    review_operation_flow_plan_quality,
    ...
    def invoke(self, request: LlmInvocationRequest) -> LlmInvocationResult:
        ...
        return LlmInvocationResult(
            ...
            metadata={
                ...
                "source": "test_operation_flow_plan_workflow",
            },
        )
...
class FailingPlanLlmService(GovernedLlmInvocationService):
    def invoke(self, request: LlmInvocationRequest) -> LlmInvocationResult:
        return LlmInvocationResult(
            ...
            metadata={"source": "test_operation_flow_plan_workflow"},
        )


def test_plan_input_triggers_workflow() -> None:
    assert detect_operation_flow_plan_request(
        "我想建一个鱼塘，500平米大，深度不低于3米，帮我设计个建设方案"
    )


def test_casual_chat_does_not_trigger_workflow() -> None:
    assert detect_operation_flow_plan_request("你好，今天心情有点闷") is False
...
def test_requirement_extraction_preserves_fishpond_entities_and_constraints() -> None:
    facts = extract_operation_flow_plan_requirements(
        OperationFlowPlanWorkflowRequestCandidate(
...
def test_terminal_display_preserves_line_breaks() -> None:
    facts = extract_operation_flow_plan_requirements(
        OperationFlowPlanWorkflowRequestCandidate(user_text="鱼塘500平米，设计建设方案")
    )
...
def test_quality_review_identifies_entity_and_scale_missing() -> None:
    ...
    review = review_operation_flow_plan_quality(formatted_without_facts, facts)
...
def test_no_live_workflow_does_not_fabricate_plan() -> None:
    result = run_operation_flow_plan_workflow(
        OperationFlowPlanWorkflowRequestCandidate(
            user_text="我要开个养鸡场，帮我设计个方案，规模500只鸡",
...
def test_controlled_live_workflow_uses_one_model_call_and_preserves_facts() -> None:
    ...
    assert result.task_run_context.stages == OPERATION_FLOW_CONTROL_STAGES
    assert result.metadata["operation_control"]["status"] == "succeeded"
    assert result.metadata["operation_control"]["config_precedence"][0] == (
