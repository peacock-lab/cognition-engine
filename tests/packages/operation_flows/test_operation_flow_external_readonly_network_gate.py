...
from cognition_operation_flows._external_readonly.network_gate import (
    ...
    build_operation_flow_external_readonly_network_gate_summary,
    evaluate_operation_flow_external_readonly_network_gate,
    operation_flow_external_readonly_network_gate_status_dict,
    operation_flow_external_readonly_network_gate_summary_status_dict,
)
...
def _valid_design(
    ...
    **overrides: object,
    ...
    return OperationFlowExternalReadonlyToolDesignCandidate(**kwargs)
...
def _valid_request(
    ...
    **overrides: object,
    ...
    kwargs.update(overrides)
    return OperationFlowExternalReadonlyNetworkRequestCandidate(**kwargs)


def _valid_approval(**overrides: object) -> OperationFlowExternalReadonlyNetworkApprovalCandidate:
    kwargs = {
    ...
    kwargs.update(overrides)
    return OperationFlowExternalReadonlyNetworkApprovalCandidate(**kwargs)


def test_google_search_network_gate_passes_without_network_call() -> None:
    gate = evaluate_operation_flow_external_readonly_network_gate(
        design=_valid_design(),
        request=_valid_request(),
        approval=_valid_approval(),
    )
    status = operation_flow_external_readonly_network_gate_status_dict(gate)
...
def test_network_gate_summary_is_sanitized_and_non_executing() -> None:
    allowed = evaluate_operation_flow_external_readonly_network_gate(
        design=_valid_design(),
        request=_valid_request(),
        approval=_valid_approval(),
    )
    blocked = evaluate_operation_flow_external_readonly_network_gate(
        design=_valid_design(
            tool_name="url_context_reference_read",
            tool_origin="url_context",
            operation_family="read",
    ...
    summary = build_operation_flow_external_readonly_network_gate_summary((allowed, blocked))
    status = operation_flow_external_readonly_network_gate_summary_status_dict(summary)
