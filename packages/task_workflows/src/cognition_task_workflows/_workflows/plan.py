"""task plan workflow skeleton for task-style terminal requests."""
...
def _invoke_plan_llm(
    ...
    *,
    ...
    facade = TwfLlmInvocationFacade(
        service=service,
        metadata={
            "source": "cognition_task_workflows._workflows.plan",
            "workflow": "twf_plan_workflow",
            "purpose": purpose,
        },
    )
...
def _reference_blocked_terminal_display(
    ...
    return "\n".join(
        [
            "task plan workflow 的 reference reader 已阻止本轮执行。",
            f"run_id: {task_context.run_id}",
            f"blocking_reasons: {reasons or 'unknown'}",
            "未调用模型，未执行外部工具。",
        ]
