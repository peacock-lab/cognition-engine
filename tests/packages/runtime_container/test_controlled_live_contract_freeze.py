...
from runtime_container.entrypoints import cognition
...
def test_cli_top_level_contract_is_frozen() -> None:
    assert cognition.ALLOWED_TOP_LEVEL_FIELDS == FROZEN_CLI_TOP_LEVEL_FIELDS
    assert FROZEN_FORBIDDEN_CLI_FIELDS.issubset(cognition.FORBIDDEN_TOP_LEVEL_FIELDS)
    assert cognition.ALLOWED_TOP_LEVEL_FIELDS.isdisjoint(
        cognition.FORBIDDEN_TOP_LEVEL_FIELDS
    )
...
def test_cli_output_boundary_rejects_nested_llm_audit_and_profile_payloads() -> None:
    ...
    assert cognition._violates_output_boundary(output) is True
    assert cognition._whitelist_output(output) == {
        "product": "Cognition System / 认知系统",
