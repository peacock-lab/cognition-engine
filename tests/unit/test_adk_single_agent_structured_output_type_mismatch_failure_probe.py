#!/usr/bin/env python3
"""ADK 单 agent 结构化输出类型不匹配失败路径最小验证脚本测试。"""
...
from engine.analyzer.probe_adk_single_agent_structured_output_type_mismatch_failure import (  # noqa: E402
    AGENT_NAME,
    APP_NAME,
...
def test_extract_node_name_strips_run_id() -> None:
    assert extract_node_name("single_agent_structured_output_type_mismatch_failure_node@1") == (
        "single_agent_structured_output_type_mismatch_failure_node"
    )
    assert extract_node_name(
        "wf@1/single_agent_structured_output_type_mismatch_failure_node@2"
    ) == "single_agent_structured_output_type_mismatch_failure_node"
...
def test_serialize_event_extracts_text_without_state_delta() -> None:
    event = SimpleNamespace(
        ...
        node_info=SimpleNamespace(path="single_agent_structured_output_type_mismatch_failure_node@1"),
    )

    record = serialize_event(event)

    assert record["author"] == AGENT_NAME
    assert record["node_name"] == "single_agent_structured_output_type_mismatch_failure_node"
...
def test_build_success_payload_marks_failure_path_acceptance() -> None:
    payload = build_success_payload(
        ...
        error_message=(
            "1 validation error for StoryOutput\n"
            "title\n"
        ...
        event_records=[
            {
                "author": AGENT_NAME,
                "node_path": "single_agent_structured_output_type_mismatch_failure_node@1",
                "node_name": "single_agent_structured_output_type_mismatch_failure_node",
                "output": None,
                "role": "model",
    ...
    assert payload["accepted_as_minimal_single_agent_structured_output_type_mismatch_failure_path"] is True
    ...
    assert payload["entered_schema_type_mismatch_failure_semantics"] is True
    assert payload["type_mismatch_failure_observed"] is True
...
def test_build_failure_payload_keeps_boundary_warning() -> None:
    payload = build_failure_payload(
        ...
        input_text="Write a tiny story as structured output.",
        exc=RuntimeError("schema type mismatch validation was not observed"),
    )

    assert payload["result"] == "failed"
    assert payload["accepted_as_minimal_single_agent_structured_output_type_mismatch_failure_path"] is False
    assert payload["error_type"] == "RuntimeError"
    assert payload["selected_runtime_path"] == SELECTED_RUNTIME_PATH
    assert "schema 类型不匹配失败承接层" in payload["boundary_judgement"]
