...
def test_controlled_run_facade_does_not_import_cli_entrypoint_or_tests() -> None:
    source = (
        RUNTIME_CONTAINER_ROOT / "controlled_run_facade.py"
...
def test_product_gateway_only_imports_runtime_container_public_facade() -> None:
    allowed_import = re.compile(
        r"^\s*from\s+runtime_container\.controlled_run_facade\s+import\s+",
        re.MULTILINE,
    )
    forbidden_import = re.compile(
        r"^\s*(?:from|import)\s+runtime_container"
        r"(?!\.controlled_run_facade\s+import\s+)",
        re.MULTILINE,
    )
    forbidden_symbols = (
    ...
    runtime_container_imports = 0
    for source_path in PRODUCT_GATEWAY_ROOT.rglob("*.py"):
        source = source_path.read_text(encoding="utf-8")
        runtime_container_imports += len(allowed_import.findall(source))
        assert forbidden_import.search(source) is None, source_path
        for symbol in forbidden_symbols:
            assert symbol not in source, (source_path, symbol)

    assert runtime_container_imports >= 1
