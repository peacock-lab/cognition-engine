...
from cognition_engine.artifacts import bind_adk_file_artifacts
...
def build_workflow_result(
    ...
    *,
    ...
    if result["status"] in {"success", "partial_success"}:
        ...
        bind_adk_file_artifacts(result)
        result["control_plane"] = build_control_plane_bundle(result)
    return result
...
def _empty_validation() -> Dict[str, bool]:
    return {
        ...
        "adk_session_id_mapped": False,
        "adk_invocation_mapped": False,
        "adk_invocation_bound": False,
        "adk_invocation_id_mapped": False,
        "adk_invocation_event_count": 0,
        "adk_invocation_mismatch": False,
        "adk_event_fields_bound": False,
        "adk_event_coverage_available": False,
        "adk_event_error_count": 0,
        "adk_event_interrupted_count": 0,
        "project_execution_id_mapped": False,
        "project_context_id_mapped": False,
        "project_invocation_id_mapped": False,
        "adk_events_mapped": False,
        "adk_runner_events_mapped": False,
        "business_step_events_mapped": False,
        "adk_artifacts_mapped": False,
        "business_artifact_refs_mapped": False,
        "workflow_summary_artifact_mapped": False,
        "output_files_mapped": False,
        "metadata_files_mapped": False,
        "legacy_fallback_used": False,
    }
...
- event_summary_source: `{result.get("event_summary", {}).get("source", "")}`
- adk_event_count: `{result.get("event_summary", {}).get("adk_event_count", 0)}`
- business_event_count: `{result.get("event_summary", {}).get("business_event_count", 0)}`
- event_boundary: `ADK Runner events + business step event mapping`
- artifact_mapping_status: `business_artifact_mapping`
- artifact_ref_count: `{len(result.get("artifact_refs", []))}`
- workflow_output_file: `{result.get("output_file") or ""}`
- workflow_metadata_file: `{result.get("metadata_file") or ""}`
- artifact_boundary: `business artifact mapping over cognition-engine outputs / metadata`
