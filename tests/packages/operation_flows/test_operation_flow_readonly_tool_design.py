...
from cognition_operation_flows._tools.readonly_tool_design import (
    OPERATION_FLOW_READONLY_TOOL_ALLOWED_OPERATIONS,
    OPERATION_FLOW_READONLY_TOOL_ALLOWED_ORIGINS,
    OperationFlowReadonlyToolDesignCandidate,
    build_operation_flow_readonly_tool_design_summary,
    review_operation_flow_readonly_tool_design,
    operation_flow_readonly_tool_design_summary_status_dict,
)


def test_adk_mcp_readonly_operations_are_design_allowed_without_runtime() -> None:
    summary = build_operation_flow_readonly_tool_design_summary(
        (
            OperationFlowReadonlyToolDesignCandidate(
    ...
    status = operation_flow_readonly_tool_design_summary_status_dict(summary)
    ...
    assert status["allowed_origins"] == sorted(OPERATION_FLOW_READONLY_TOOL_ALLOWED_ORIGINS)
...
def test_write_or_execute_operation_is_blocked_for_readonly_design() -> None:
    review = review_operation_flow_readonly_tool_design(
        OperationFlowReadonlyToolDesignCandidate(
...
def test_google_search_and_url_context_remain_deferred_candidates() -> None:
    search_review = review_operation_flow_readonly_tool_design(
        OperationFlowReadonlyToolDesignCandidate(
    ...
    url_review = review_operation_flow_readonly_tool_design(
        OperationFlowReadonlyToolDesignCandidate(
