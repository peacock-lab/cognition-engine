...
def test_four_layer_minimal_loop_builds_evidence_bundle_from_real_runtime_result() -> None:
    ...
    class MinimalNode(BaseNode):
        async def _run_impl(self, *, ctx: Context, node_input: Any):
            yield Event(
                ...
                output={"ok": True, "node": self.name, "echo": "hello from four-layer loop"},
            )

    workflow = Workflow(
        name="four_layer_minimal_loop_workflow",
        edges=[(START, MinimalNode(name="four_layer_node"))],
    )
    runtime_runner = runtime_facade.build_standard_runtime_runner(
        ...
        workflow_runner=AdkWorkflowRunner(
            workflow=workflow,
            app_name="test_four_layer_minimal_loop",
            user_id="four-layer-loop-user",
        ),
    )

    runtime_result = runtime_runner.run(
        contract_runtime.RuntimeInput(
            runtime_id="runtime-four-layer-001",
            workflow_ref=contract_runtime.WorkflowRef(
                workflow_id="workflow-four-layer-001",
                name="four_layer_minimal_loop_workflow",
            ),
            invocation_ref=contract_runtime.InvocationRef(invocation_id="requested-loop-001"),
            input_payload={"message": "hello from four-layer loop"},
    ...
    assert evidence_bundle.runtime_id == "runtime-four-layer-001"
    ...
    assert "four_layer_minimal_loop_workflow@1/four_layer_node@1" in evidence_bundle.event_trace.node_paths
    assert "four_layer_minimal_loop_workflow" in evidence_bundle.event_trace.authors
    ...
    assert evidence_bundle.invocation.workflow_id == "workflow-four-layer-001"
