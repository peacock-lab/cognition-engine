    ...
    def invoke(self, request: LlmInvocationRequest) -> LlmInvocationResult:
        ...
        return LlmInvocationResult(
            ...
            metadata={
                ...
                "source": "test_product_gateway_operation_flow_execution",
            },
        )
...
def test_product_gateway_executes_operation_flow_plan_draft_through_product_entry() -> None:
    route_projection = build_internal_operation_flow_route_projection(
        {
            "request_id": "operation_flow-execution-plan",
            "sanitized_user_text": "我要建一个鱼塘，500平米大，帮我设计建设方案",
            "chat_session_id": "session-product-gateway-execution",
            "turn_index": 1,
        }
    )
    request_draft = build_internal_operation_flow_plan_request_draft(
        sanitized_user_text="我要建一个鱼塘，500平米大，帮我设计建设方案",
        chat_session_id="session-product-gateway-execution",
        turn_index=1,
        route_summary=route_projection.model_dump(),
        governance_refs=InternalOperationFlowGovernanceRefs(
            approval_ref="approval://operation_flow-execution-plan",
            audit_ref="audit://operation_flow-execution-plan",
            sanitized_evidence_ref="evidence://operation_flow-execution-plan",
            governance_summary_output_ref="artifact://operation_flow-execution-plan",
        ),
...
def test_product_gateway_skips_unmatched_operation_flow_route_without_execution() -> None:
    route_projection = build_internal_operation_flow_route_projection(
        {
            "request_id": "operation_flow-execution-skip",
    ...
    assert result.product_response.warnings == ["operation_flow_route_not_matched"]
    assert result.workflow_result is None


def test_product_gateway_blocks_live_operation_flow_when_provider_not_injected() -> None:
    route_projection = build_internal_operation_flow_route_projection(
        {
            "request_id": "operation_flow-execution-live-blocked",
            "sanitized_user_text": "我要建一个鱼塘，500平米大，帮我设计建设方案",
            "chat_session_id": "session-product-gateway-execution",
            "turn_index": 3,
        }
    ...
    assert result.product_response.warnings == [
        "operation_flow_live_llm_provider_required"
    ]
    assert result.product_response.metadata["provider_resolution_blocked"] is True
    assert result.workflow_result is None


def test_product_gateway_executes_live_operation_flow_with_injected_service() -> None:
    service = FakeProductGatewayLlmService(
        "1. 场地规划\n2. 建设步骤\n3. 运维安排"
    )
    route_projection = build_internal_operation_flow_route_projection(
        {
            "request_id": "operation_flow-execution-live-service",
            "sanitized_user_text": "我要建一个鱼塘，500平米大，帮我设计建设方案",
            "chat_session_id": "session-product-gateway-execution",
            "turn_index": 4,
        }
...
def test_product_gateway_executes_live_operation_flow_with_injected_factory() -> None:
    service = FakeProductGatewayLlmService(
        "1. 场地规划\n2. 建设步骤\n3. 运维安排"
    )
    ...
    route_projection = build_internal_operation_flow_route_projection(
        {
            "request_id": "operation_flow-execution-live-factory",
            "sanitized_user_text": "我要建一个鱼塘，500平米大，帮我设计建设方案",
            "chat_session_id": "session-product-gateway-execution",
            "turn_index": 5,
        }
    ...
    assert config_selection.config_root == "config/operation_flow"
    ...
    assert live_llm_options.metadata["request_id"] == (
        "operation_flow-execution-live-factory"
    )
    assert len(service.requests) == 1


def test_product_gateway_operation_flow_execution_consumes_external_readonly_refs(
    tmp_path: Path,
) -> None:
    ...
    route_projection = build_internal_operation_flow_route_projection(
        {
            "request_id": "operation_flow-execution-external-readonly-refs",
    ...
    request_draft = build_internal_operation_flow_reference_review_request_draft(
        sanitized_user_text="请审查这份外部只读证据摘要，指出问题和建议",
        chat_session_id="session-product-gateway-external-readonly",
        turn_index=6,
        route_summary=route_projection.model_dump(),
        governance_refs=InternalOperationFlowGovernanceRefs(
            approval_ref="approval://operation_flow-execution-external-readonly-refs",
            audit_ref="audit://operation_flow-execution-external-readonly-refs",
            sanitized_evidence_ref=(
                "evidence://operation_flow-execution-external-readonly-refs"
            ),
            governance_summary_output_ref=(
                "artifact://operation_flow-execution-external-readonly-refs"
            ),
        ),
