"""CLI run workspace evidence audit workflow."""
...
from runtime_container.cli_run_workspace import (
    CLI_RUN_WORKSPACE_SCHEMA_VERSION,
    CLI_RUN_WORKSPACE_SUBDIRS,
...
CLI_RUN_WORKSPACE_EVIDENCE_AUDIT_WORKFLOW_NAME = (
    "cli_run_workspace_evidence_audit_workflow"
)
CLI_RUN_WORKSPACE_EVIDENCE_AUDIT_TASK_KIND = "run_workspace_evidence_audit"
CLI_RUN_WORKSPACE_EVIDENCE_AUDIT_TEMPLATE_VERSION = (
...
class CliRunWorkspaceEvidenceAuditWorkflowRequestCandidate:
...
@dataclass(frozen=True)
class CliRunWorkspaceEvidenceAuditFactsCandidate:
...
@dataclass(frozen=True)
class CliRunWorkspaceAuditTargetCandidate:
...
@dataclass(frozen=True)
class CliRunWorkspaceAuditFileCandidate:
...
@dataclass(frozen=True)
class CliRunWorkspaceAuditFindingCandidate:
...
@dataclass(frozen=True)
class CliRunWorkspaceEvidenceAuditContextCandidate:
    """Sanitized run workspace evidence audit context."""

    status: str
    audit_result: str
    target: CliRunWorkspaceAuditTargetCandidate
    files: tuple[CliRunWorkspaceAuditFileCandidate, ...]
    findings: tuple[CliRunWorkspaceAuditFindingCandidate, ...]
...
@dataclass(frozen=True)
class CliRunWorkspaceEvidenceAuditWorkflowResultCandidate:
    """Final run workspace evidence audit workflow result."""

    triggered: bool
    terminal_display_text: str
    request: CliRunWorkspaceEvidenceAuditWorkflowRequestCandidate
    facts: CliRunWorkspaceEvidenceAuditFactsCandidate
    audit_context: CliRunWorkspaceEvidenceAuditContextCandidate
...
def detect_cli_run_workspace_evidence_audit_request(
    user_text: str,
    *,
...
def extract_cli_run_workspace_evidence_audit_facts(
    request: CliRunWorkspaceEvidenceAuditWorkflowRequestCandidate,
) -> CliRunWorkspaceEvidenceAuditFactsCandidate:
    ...
    return CliRunWorkspaceEvidenceAuditFactsCandidate(
...
def run_cli_run_workspace_evidence_audit_workflow(
    request: CliRunWorkspaceEvidenceAuditWorkflowRequestCandidate,
) -> CliRunWorkspaceEvidenceAuditWorkflowResultCandidate:
    """Run the governed run workspace evidence audit workflow."""

    facts = extract_cli_run_workspace_evidence_audit_facts(request)
    ...
    if not task_context.preflight.allowed:
        ...
        return CliRunWorkspaceEvidenceAuditWorkflowResultCandidate(
            triggered=True,
            ...
            metadata={
                "workflow": CLI_RUN_WORKSPACE_EVIDENCE_AUDIT_WORKFLOW_NAME,
                **_task_context_metadata(task_context),
            },
        )

    target = resolve_cli_run_workspace_audit_target(request)
    audit_context = build_cli_run_workspace_evidence_audit_context(request, facts, target)
    display = format_cli_run_workspace_evidence_audit_for_terminal(audit_context)
...
def resolve_cli_run_workspace_audit_target(
    request: CliRunWorkspaceEvidenceAuditWorkflowRequestCandidate,
) -> CliRunWorkspaceAuditTargetCandidate:
    ...
    if target_path is not None and target_path.is_dir():
        if manifest_path is None or not manifest_path.is_file():
            ...
            if schema != CLI_RUN_WORKSPACE_SCHEMA_VERSION:
    ...
    return CliRunWorkspaceAuditTargetCandidate(
...
def build_cli_run_workspace_evidence_audit_context(
    request: CliRunWorkspaceEvidenceAuditWorkflowRequestCandidate,
    facts: CliRunWorkspaceEvidenceAuditFactsCandidate,
    target: CliRunWorkspaceAuditTargetCandidate,
) -> CliRunWorkspaceEvidenceAuditContextCandidate:
    ...
    return CliRunWorkspaceEvidenceAuditContextCandidate(
        status="succeeded",
        ...
        metadata={
            "template_version": CLI_RUN_WORKSPACE_EVIDENCE_AUDIT_TEMPLATE_VERSION,
...
def format_cli_run_workspace_evidence_audit_for_terminal(
    audit_context: CliRunWorkspaceEvidenceAuditContextCandidate,
) -> str:
...
def _audit_workspace_files(
    ...
    tuple[CliRunWorkspaceAuditFileCandidate, ...],
    tuple[CliRunWorkspaceAuditFindingCandidate, ...],
]:
    files: list[CliRunWorkspaceAuditFileCandidate] = []
    findings: list[CliRunWorkspaceAuditFindingCandidate] = []
    refs_by_kind = {
    ...
    for kind, refs in refs_by_kind.items():
        for ref in refs:
            ...
            if relative_path is None:
                findings.append(
                    CliRunWorkspaceAuditFindingCandidate(
                        severity="medium",
...
def _audit_one_file(root: Path, kind: str, relative_path: str) -> CliRunWorkspaceAuditFileCandidate:
    resolved = _resolve_audit_relative_path(root, relative_path)
    ref = f"{kind}://run-workspace/{relative_path}"
    if resolved is None:
        return CliRunWorkspaceAuditFileCandidate(
            kind=kind,
...
def _audit_structure(
    root: Path,
    manifest: Mapping[str, Any],
    files: Sequence[CliRunWorkspaceAuditFileCandidate],
) -> tuple[dict[str, Any], tuple[CliRunWorkspaceAuditFindingCandidate, ...]]:
    findings: list[CliRunWorkspaceAuditFindingCandidate] = []
    subdir_status = {
        subdir: (root / subdir).is_dir() for subdir in CLI_RUN_WORKSPACE_SUBDIRS
    }
...
def _create_audit_output_run_workspace(
    ...
    if request.run_workspace_root and _audit_output_root_inside_target(
        ...
        return CliRunWorkspaceStateCandidate(
            ...
            subdirs=CLI_RUN_WORKSPACE_SUBDIRS,
...
def _resolve_audit_relative_path(root: Path, relative_path: str) -> Path | None:
    ...
    if path.is_absolute() or not parts or ".." in parts:
        return None
    if parts[0] not in set(CLI_RUN_WORKSPACE_SUBDIRS):
        return None
...
def _audit_result_from_findings(
    findings: Sequence[CliRunWorkspaceAuditFindingCandidate],
) -> str:
