...
from cognition_task_workflows._workflows.reference_review import (
    TWF_REFERENCE_REVIEW_TEMPLATE_VERSION,
...
from cognition_task_workflows._core.control import TWF_CONTROL_STAGES
    ...
    def invoke(self, request: LlmInvocationRequest) -> LlmInvocationResult:
        ...
        return LlmInvocationResult(
            ...
            metadata={
                ...
                "source": "test_twf_reference_review_workflow",
            },
        )
...
class FailingReferenceReviewLlmService(GovernedLlmInvocationService):
    ...
    def invoke(self, request: LlmInvocationRequest) -> LlmInvocationResult:
        ...
        return LlmInvocationResult(
            ...
            metadata={"source": "test_twf_reference_review_workflow"},
        )


def test_reference_review_detector_requires_reference_and_review_intent() -> None:
    assert detect_twf_reference_review_request(
        "请审查这些资料，指出问题和建议",
...
def test_no_live_reference_review_reads_reference_and_outputs_evidence(
    ...
    reference.write_text(
        "通用 task workflow 阶段收口。\n"
        "Agent runtime 继续关闭。\n"
        "下一步建议进入真实验收。\n",
        encoding="utf-8",
    )

    result = run_twf_reference_review_workflow(
        TwfReferenceReviewWorkflowRequestCandidate(
    ...
    assert result.task_run_context.workflow_name == TWF_REFERENCE_REVIEW_WORKFLOW_NAME
...
def test_controlled_live_reference_review_uses_one_model_call(
    ...
    reference = tmp_path / "review.md"
    reference.write_text("当前主线是第二类真实 task workflow。\n", encoding="utf-8")
    service = FakeReferenceReviewLlmService(
        json.dumps(
            {
                "conclusion": "资料符合当前主线。",
                "evidence_basis": ["资料声明当前主线是第二类真实 task workflow。"],
                "issues": [],
...
def test_no_live_reference_review_outputs_terminology_annotations_when_requested(
    ...
    assert TWF_REFERENCE_REVIEW_TEMPLATE_VERSION == "reference_review_template_v2"
...
def test_reference_review_persists_run_workspace_layers(tmp_path: Path) -> None:
    ...
    assert result_payload["workflow"] == TWF_REFERENCE_REVIEW_WORKFLOW_NAME
