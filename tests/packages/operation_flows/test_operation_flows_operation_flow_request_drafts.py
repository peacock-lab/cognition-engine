...
from cognition_operation_flows._requests.drafts import (  # noqa: E402
    OPERATION_FLOW_REQUEST_DRAFT_SCHEMA_VERSION,
...
def test_plan_request_draft_is_channel_neutral_candidate_only() -> None:
    draft = build_operation_flow_plan_request_draft(
        ...
        governance_refs=OperationFlowGovernanceRefsCandidate(
        ...
        route_summary={
            "source": "product_gateway._operation_flows.route",
            "matched": True,
        },
    ...
    assert draft.schema_version == OPERATION_FLOW_REQUEST_DRAFT_SCHEMA_VERSION
    assert draft.workflow_name == OPERATION_FLOW_PLAN_WORKFLOW_NAME
    assert draft.task_kind == OPERATION_FLOW_PLAN_TASK_KIND
    ...
    status = operation_flow_workflow_request_draft_status_dict(draft)
    assert status["workflow_name"] == OPERATION_FLOW_PLAN_WORKFLOW_NAME
...
def test_request_draft_rejects_boundary_drift(
    ...
    payload = {
        ...
        "turn_input": build_operation_flow_plan_request_draft(
    ...
    with pytest.raises(ValueError):
        OperationFlowWorkflowRequestDraftCandidate(**payload)
...
def test_operation_flows_request_draft_has_no_runtime_or_channel_imports() -> None:
    source = (
        PACKAGE_ROOT
        / "src"
...
def test_operation_flows_package_flags_request_draft_candidate() -> None:
