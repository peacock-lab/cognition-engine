...
def test_task_run_context_records_control_structure_and_refs() -> None:
    ...
    assert context.run_id == "operation-flow-plan-workflow-cli-plan-test-turn-003"
    assert context.stages == OPERATION_FLOW_CONTROL_STAGES
    assert context.config_precedence == OPERATION_FLOW_CONFIG_PRECEDENCE
    ...
    assert context.workspace.workspace_ref.startswith(
        "run-workspace://operation-flow-plan-workflow/"
    )
...
def test_task_status_dict_is_sanitized_and_finalizable() -> None:
    ...
    finalized = finalize_operation_flow_run_context(
        context,
        status="succeeded",
        artifact_refs=("candidate-artifact://run/result",),
        evidence_refs=("evidence://extra",),
    )
    status = operation_flow_run_context_status_dict(finalized)

    assert status["status"] == "succeeded"
    assert status["run_id"] == "operation-flow-plan-workflow-session-with-spaces-turn-001"
    ...
    assert status["config_precedence"] == list(OPERATION_FLOW_CONFIG_PRECEDENCE)
