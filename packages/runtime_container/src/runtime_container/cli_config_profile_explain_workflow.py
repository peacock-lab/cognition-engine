"""CLI config profile explain workflow for governed configuration diagnostics."""
...
CLI_CONFIG_PROFILE_EXPLAIN_WORKFLOW_NAME = "cli_config_profile_explain_workflow"
CLI_CONFIG_PROFILE_EXPLAIN_TASK_KIND = "config_profile_explain"
CLI_CONFIG_PROFILE_EXPLAIN_TEMPLATE_VERSION = "config_profile_explain_template_v1"
...
class CliConfigProfileExplainWorkflowRequestCandidate:
    """Request entering the CLI config profile explain workflow."""
    ...
    cli_explicit_args: Mapping[str, Any] = field(default_factory=dict)
...
@dataclass(frozen=True)
class CliConfigProfileExplainFactsCandidate:
...
@dataclass(frozen=True)
class CliConfigProfileExplainValueCandidate:
...
@dataclass(frozen=True)
class CliConfigProfileExplainContextCandidate:
    ...
    effective_values: tuple[CliConfigProfileExplainValueCandidate, ...]
...
@dataclass(frozen=True)
class CliConfigProfileExplainWorkflowResultCandidate:
    """Final workflow result returned to cognition chat."""

    triggered: bool
    terminal_display_text: str
    request: CliConfigProfileExplainWorkflowRequestCandidate
    facts: CliConfigProfileExplainFactsCandidate
    explain_context: CliConfigProfileExplainContextCandidate
...
def detect_cli_config_profile_explain_request(user_text: str) -> bool:
...
def extract_cli_config_profile_explain_facts(
    request: CliConfigProfileExplainWorkflowRequestCandidate,
) -> CliConfigProfileExplainFactsCandidate:
    ...
    return CliConfigProfileExplainFactsCandidate(
...
def run_cli_config_profile_explain_workflow(
    request: CliConfigProfileExplainWorkflowRequestCandidate,
) -> CliConfigProfileExplainWorkflowResultCandidate:
    """Run the governed CLI config profile explain workflow."""

    facts = extract_cli_config_profile_explain_facts(request)
    ...
    if not task_context.preflight.allowed:
        ...
        return CliConfigProfileExplainWorkflowResultCandidate(
            triggered=True,
            ...
            metadata={
                "workflow": CLI_CONFIG_PROFILE_EXPLAIN_WORKFLOW_NAME,
    ...
    explain_context = build_cli_config_profile_explain_context(request, facts)
    display = format_cli_config_profile_explain_for_terminal(explain_context)
...
def build_cli_config_profile_explain_context(
    request: CliConfigProfileExplainWorkflowRequestCandidate,
    facts: CliConfigProfileExplainFactsCandidate | None = None,
) -> CliConfigProfileExplainContextCandidate:
    """Build a sanitized explanation context from parsed runtime config views."""

    resolved_facts = facts or extract_cli_config_profile_explain_facts(request)
    ...
    effective_values = (
        CliConfigProfileExplainValueCandidate(
    ...
    return CliConfigProfileExplainContextCandidate(
        status="succeeded",
        ...
        precedence=CLI_TASK_CONFIG_PRECEDENCE,
        ...
        metadata={
            "template_version": CLI_CONFIG_PROFILE_EXPLAIN_TEMPLATE_VERSION,
...
def format_cli_config_profile_explain_for_terminal(
    explain_context: CliConfigProfileExplainContextCandidate,
) -> str:
    ...
    lines = [
        ...
        "",
        "覆盖关系",
        "1. CLI 显式参数 > 会话参数 > profile/config > 默认值",
        "",
        "当前有效配置",
        (
        ...
        "建议动作",
        "1. 若要改变配置，请另开配置修改任务。",
        "2. 若要验证当前配置，请执行 no-live CLI smoke。",
...
def _tool_and_reference_summary(
    ...
    source = "cli_explicit_args" if request.tool_exposure_profile else (
    ...
    return (
        {
            ...
            "config_precedence": list(CLI_TASK_CONFIG_PRECEDENCE),
...
def _run_workspace_summary(
    ...
    cli_keys = set(request.cli_explicit_args)
    cli_workspace_requested = bool(
        {
    ...
    source = "cli_explicit_args" if cli_workspace_requested else (
...
def _live_llm_summary(
    ...
    source = "cli_explicit_args" if cli_live_requested else (
