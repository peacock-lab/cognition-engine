...
from cognition_task_workflows._core.run_workspace import (  # noqa: E402
...
def test_run_workspace_creates_standard_directories_and_manifest(tmp_path) -> None:
    policy = build_twf_run_workspace_policy(workspace_root=tmp_path)
    ...
    assert manifest["schema_version"] == "twf-run-workspace.v1"
...
def test_run_workspace_writes_refs_and_final_manifest(tmp_path) -> None:
    ...
    workspace, evidence_write = write_twf_run_workspace_json(
        workspace,
        relative_path="evidence/reference_context.json",
        payload={"status": "succeeded"},
        kind="evidence",
    )
    workspace, artifact_write = write_twf_run_workspace_text(
        workspace,
        relative_path="artifacts/terminal_display.txt",
        text="鱼塘建设方案\n",
        kind="artifact",
    )
    workspace, result_write = write_twf_run_workspace_json(
        workspace,
        relative_path="results/workflow_result.json",
        payload={"status": "succeeded"},
        kind="result",
    )
    workspace = finalize_twf_run_workspace(workspace, status="succeeded")
    status = twf_run_workspace_status_dict(workspace)
...
def test_run_workspace_blocks_path_traversal_and_sensitive_paths(tmp_path) -> None:
    ...
    _, traversal = write_twf_run_workspace_text(
        workspace,
        relative_path="../escape.txt",
        text="bad",
        kind="artifact",
    )
    _, sensitive = write_twf_run_workspace_text(
        workspace,
...
def test_run_workspace_cleanup_delete_on_success(tmp_path) -> None:
    ...
    workspace, cleanup = cleanup_twf_run_workspace(workspace, status="succeeded")
